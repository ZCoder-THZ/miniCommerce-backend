...
@section('content')
    <div class="p-4 sm:ml-64">
        <div class="p-4  border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700">
            ...
            <h1 class="text-left p-4 mb-4 text-black bg-green-100 rounded-tl-md rounded-tr-md">Edit Item</h1>
            <form action="{{ route('editItem', $item->id) }}" class="" enctype="multipart/form-data" method="post">
                @csrf

                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div class="  rounded   ">
                        <h1 class="text-left pl-4"> Informations</h1>

                        <div class="mb-3 p-4">
                            <label for="email" class="block mb-2 text-sm font-medium text-gray-900">Item
                                name</label>
                            <input type="text" id="email" name="name" value="{{ $item->item_name }}"
                            ...
                            <select id="countries" name="itemCondition"
                                ...
                                @foreach ($itemConditions as $key => $itemCondition)
                                    <option value="{{ $itemCondition->item_condition }}"
                                        @if ($itemCondition->item_condition == $item->item_condition) selected @endif>
                            ...
                            <select id="countries" name="itemType"
                                ...
                                @foreach ($itemTypes as $key => $itemType)
                                    <option value="{{ $itemType->item_type }}"
                                        @if ($itemType->item_type == $item->item_type) selected @endif>
                        ...
                        <div class="mb-3 p-4">
                            ...
                            <div class="flex items-center mb-4">
                                ...
                                <input id="default-checkbox" type="checkbox"
                                    @if ($item->status == 'avail') checked @endif onchange="updateValue(this)"
                                    name="status" value="avail"
                    ...
                    <div class=" rounded ">
                        ...
                        <div class="mb-3 p-4">
                            <div class="flex">
                                ...
                                <input type="number" id="website-admin" name="phone" value="{{ $item->phone }}"
                        ...
                        <div class="mb-3 p-4">
                            ...
                            <textarea rows="6" name="address"
                                class="block p-2.5 h-40 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500  dark:placeholder-gray-400 dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                placeholder="Write your thoughts here...">{{ $item->address }}</textarea>
                            @error('address')
                                <h1 class="text-red-400">{{ $message }}</h1>
                            @enderror
    ...
    <script>
        function updateValue(checkbox) {
            if (checkbox.checked) {
                checkbox.value = "avail";
                console.log(checkbox.value)
            } else {
                checkbox.value = "unavail";
                console.log(checkbox.value)
